...
6. **Verify Coverage:** Run coverage reports using the project's chosen tools. For example, in a Python project, this might look like:
   ...
   ```bash
   pytest --cov=ai_travel_planner --cov-report=html
   ```
...
3.  **Execute Automated Tests with Proactive Debugging:**
    ...
    - **Example Announcement:** "I will now run the automated test suite to verify the phase. **Command:** `CI=true pixi run test`"
...
4.  **Propose a Detailed, Actionable Manual Verification Plan:**
    ...
    - The plan you present to the user **must** follow this format:
      ...
      **Manual Verification Steps:**
      1.  **Start the development server with the command:** `pixi run app`
      2.  **Open your browser to:** `http://localhost:8501`
      3.  **Confirm that you see:** [specific expected outcome]
      ```
      ...
      **Manual Verification Steps:**
      ...
      2.  **Execute the following command in your terminal:** `[specific test command]`
      3.  **Confirm that you receive:** [specific expected response]
      ```
...
- Mock external dependencies (AI providers, HTTP calls).
...
```bash
git commit -m "feat(itinerary): Add diff-based update functionality"
git commit -m "fix(pdf): Correct image scaling in magazine template"
git commit -m "test(agents): Add tests for Claude agent error handling"
```
...
2. Restore from latest backup/session file
...
1. Switch to alternative provider in Settings
2. Document which provider is unavailable
3. Continue work with available provider
4. Monitor for provider recovery
